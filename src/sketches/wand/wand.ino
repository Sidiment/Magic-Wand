...
void loop()
{
    // Check for switch press on GPIO 8 (active LOW)
    static bool lastButtonState = HIGH;
    bool buttonState = digitalRead(8);
    if (lastButtonState == HIGH && buttonState == LOW && !capturing) {
        // Button was just pressed
        Serial.println("Switch pressed: Starting gesture capture...");
        sample_count = 0;
        capturing = true;
...
void print_inference_result(ei_impulse_result_t result) {
    ...
    if (max_index != -1) {
        ...
        // Example: 0 = first label, 1 = second label, 2 = third label
        // Adjust the mapping as needed for your model's labels
        digitalWrite(3, LOW);
        digitalWrite(4, LOW);
        digitalWrite(5, LOW);
        if (max_index == 0) {
            digitalWrite(3, HIGH); // LED on GPIO 3
        } else if (max_index == 1) {
            digitalWrite(4, HIGH); // LED on GPIO 4
        } else if (max_index == 2) {
